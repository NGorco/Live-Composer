 ...
 ***********************************/
	...
	function dslc_hide_composer()
	{
		...
		jQuery('.dslca-container').css({bottom: -jQuery('.dslca-container').outerHeight()});
	...
	function dslc_generate_filters(){
		...
		jQuery('.dslca-section:visible .dslca-section-title-filter-options')
			.html(filtersHTML)
			.css('background', jQuery('.dslca-section:visible')
			     .data('bg'));
	}
	...
	function dslc_drag_and_drop(){
		...
		jQuery('.dslc-content').sortable({
			...
			start: function(e, ui){
				...
				ui.placeholder.html('<span class="dslca-placeholder-help-text"><span class="dslca-placeholder-help-text-inner">' +
				                    DSLCString.str_row_helper_text + '</span></span>');
		...
		jQuery('.dslc-modules-section-inner').sortable({
			...
			start: function(e, ui){
				...
				// Placeholder
				ui.placeholder.html('<span class="dslca-placeholder-help-text"><span class="dslca-placeholder-help-text-inner">' +
				                    DSLCString.str_area_helper_text + '</span></span>');
				...
				jQuery('body').removeClass('dslca-drag-not-in-progress')
			...
			stop: function(e, ui){
				...
				jQuery('body').removeClass('dslca-drag-in-progress dslca-modules-area-drag-in-progress')
		...
		jQuery('.dslc-modules-area').droppable({
			...
			drop: function(event, ui){
				...
				/** LC Ferrum start **/
				if(DSLC.ModulesManager.ModulesInfo[moduleID] && DSLC.ModulesManager.ModulesInfo[moduleID].version == 2){

					try{

						DSLC.ModulesManager.CreateModule(moduleID, modulesArea);
					}catch(e){

						DSLC.ModalWindow({
							title: DSLCString.str_add_module_create_error_title,
							content: DSLCString.str_add_module_render_error,
						});
				...
				if(!(moduleID == 'DSLC_M_A' ||
				   ...
				   modulesArea.closest('#dslc-header').length ||
				   modulesArea.closest('#dslc-footer').length)){
...
/*
			...
			start: function(e, ui){
				...
				ui.placeholder.html('<span class="dslca-placeholder-help-text"><span class="dslca-placeholder-help-text-inner">' +
				                    ui.item.find('.dslc-sortable-helper-icon').data('title') + '</span></span>');
		...
		$(document).on('click', '.dslca-currently-editing', function(){
			...
			if(activeElement){
				...
				$('html, body').animate({scrollTop: newOffset}, 300, function(){
					...
					activeElement
					.animate({'outline-color': outlineColor}, 70)
					.animate({'outline-color': 'transparent'}, 70, function(){
		...
		$(document).on('click', '.dslca-close-composer-hook', function(e){
			...
			if(!$('body').hasClass('dslca-saving-in-progress')){
				dslc_js_confirm('disable_lc', '<span class="dslca-prompt-modal-title">' +
				                DSLCString.str_exit_title + '</span><span class="dslca-prompt-modal-descr">' +
				                DSLCString.str_exit_descr + '</span>', $(this).attr('href'));
			}
		});
 ...
 ***********************************/
	...
	function dslc_scroller_prev(scroller){
		...
		var scrollerNew = scroller.find('.dslca-scroller-item:eq(' + scrollerCurr +  ')')
			.prevAll('.dslca-scroller-item:visible').eq(1).index();

		// One place before current
		var scrollerNewAlt = scroller.find('.dslca-scroller-item:eq(' + scrollerCurr +  ')')
			.prevAll('.dslca-scroller-item:visible').eq(0).index();
	...
	function dslc_scroller_next(scroller){
		...
		var scrollerNew = scroller.find('.dslca-scroller-item:eq(' + scrollerCurr +  ')')
			.nextAll('.dslca-scroller-item:visible').eq(1).index();

		// One place after current
		var scrollerNewAlt = scroller.find('.dslca-scroller-item:eq(' + scrollerCurr +  ')')
			.nextAll('.dslca-scroller-item:visible').eq(0).index();
 ...
 ***********************************/
	...
	jQuery(document).ready(function($){
		...
		$(document).on('click', '.dslca-prompt-modal-confirm-hook', function(e){
			...
			if(closeAtEnd) dslc_js_confirm_close();
 ...
 ***********************************/
	...
	function dslc_row_add(callback)
	{
		...
		jQuery('#dslc-main').append(jQuery(DSLC.BasicModule.prototype.optionsTemplates['new-row']));
	...
	function dslc_row_edit(row)
	{
		...
		jQuery('.dslca-modules-section-edit-field').each(function(){
			...
			var border = jQuery('.dslca-modules-section-being-edited .dslca-modules-section-settings input[data-id="border"]');
			var borderSiblings = jQuery(this).siblings('.dslca-modules-section-edit-option-checkbox-hook');
			var optionId = jQuery(this).data('id');
			var val = border ? border.val() : '';

			if( ['top','left','right','bottom'].indexOf( optionId.replace( 'border-','' ) ) > -1 ){

				jQuery(this).parent().prepend(this);
				var checked = val.split(' ').indexOf( optionId.replace( 'border-','' ) ) > -1;
				jQuery(this).prop('checked', checked);
			}
			else if(jQuery(this).hasClass('dslca-modules-section-edit-field-checkbox')){

				jQuery(this).parent().prepend(this);
				if(jQuery('.dslca-modules-section-being-edited .dslca-modules-section-settings input[data-id="' + jQuery(this).data('id') + '"]')
				.val().indexOf(jQuery(this).data('val')) > -1){
				...
				if(jQuery(this).hasClass('dslca-modules-section-edit-field-colorpicker')){
					...
					var _this = jQuery(this);
					jQuery(this).closest('.dslca-modules-section-edit-option').find('.sp-preview-inner')
						.removeClass('sp-clear-display').css({'background-color': _this.val()});
				}
			}
		});
	...
	function dslc_row_edit_slider_init()
	{
		...
		jQuery('.dslca-modules-section-edit-field-slider').each(function(){
			...
			dslcSliderTooltip,
			dslcSliderTooltipOffset,
			dslcSliderHandle,
			dslcSliderTooltipPos,
			dslcSection, dslcOptionID,
			dslcSliderExt = '',
	...
	function dslc_row_edit_cancel(callback){
		...
		jQuery('.dslca-modules-section-being-edited .dslca-modules-section-settings input').each(function(){
			...
			jQuery('.dslca-modules-section-edit-field[data-id="' + jQuery(this).data('id') + '"]')
				.val(jQuery(this).data('def')).trigger('change');
		...
		jQuery('.dslca-modules-section-being-edited')
			.removeClass('dslca-modules-section-being-edited dslca-modules-section-change-made');
	...
	function dslc_row_copy(row){
		...
		// Clone the row
		dslcModulesSectionCloned = row.clone();
		row.after(dslcModulesSectionCloned);
		...
		dslcModulesSectionCloned.find('.dslc-module-front')
		.each(function(){
	...
	jQuery(document).ready(function($){
		...
		$(document).on('click', '.dslca-edit-modules-section-hook', function(){
			...
			if(!$(this).hasClass('dslca-action-disabled')){
				...
				if($('.dslca-module-being-edited, .dslca-modules-section-being-edited').length > 0){
					...
					DSLC.ModalWindow({
						...
						confirm: function()
						{
							if($('.dslca-modules-section-being-edited').length > 0){

								$('.dslca-row-edit-save').click();
								$('.dslca-modules-section-being-edited').removeClass('dslca-modules-section-being-edited');
							}else{

								$(".dslca-module-edit-save").click();
						...
						cancel: function()
						{
							if($('.dslca-modules-section-being-edited').length > 0){
								...
								$('.dslca-row-edit-cancel').click();
								$('.dslca-modules-section-being-edited').removeClass('dslca-modules-section-being-edited');
							}else{

								$(".dslca-module-edit-cancel").click();
								$('.dslca-module-being-edited').removeClass('dslca-module-being-edited');
							}
		...
		$(document).on('click', '.dslca-import-modules-section-hook', function(e){
			...
			if(!$(this).hasClass('dslca-action-disabled')){
				...
				.html('<span class="dslc-icon dslc-icon-ok"></span><span>' + DSLCString.str_import +
				      '</span><div class="dslca-loading followingBallsGWrap"><div class="followingBallsG_1 followingBallsG"></div>' +
				      '<div class="followingBallsG_2 followingBallsG"></div><div class="followingBallsG_3 followingBallsG"></div>' +
				      '<div class="followingBallsG_4 followingBallsG"></div></div>');

				dslc_js_confirm('import_modules_section', '<span class="dslca-prompt-modal-title">' +
				                DSLCString.str_import_row_title + '</span><span class="dslca-prompt-modal-descr">' +
				                DSLCString.str_import_row_descr + ' <br><br><textarea></textarea></span>', $(this));
			}
		});
		...
		$(document).on('click', '.dslca-delete-modules-section-hook', function(e){
			...
			if(!$(this).hasClass('dslca-action-disabled')){
				dslc_js_confirm('delete_modules_section', '<span class="dslca-prompt-modal-title">' +
				                DSLCString.str_del_row_title + '</span><span class="dslca-prompt-modal-descr">' +
				                DSLCString.str_del_row_descr + '</span>', $(this));
			}
		});
		...
		$(document).on('click', '.dslca-export-modules-section-hook', function(e){
			...
			if(!$(this).hasClass('dslca-action-disabled')){
				...
				dslc_js_confirm('export_modules_section', '<span class="dslca-prompt-modal-title">' +
				                DSLCString.str_export_row_title + '</span><span class="dslca-prompt-modal-descr">' +
				                DSLCString.str_export_row_descr + ' <br><br><textarea></textarea></span>', $(this));
 ...
 ***********************************/
 	...
 	function dslc_modules_area_add(row){
		...
		setTimeout(function(){
			...
			jQuery( DSLC.BasicModule.prototype.optionsTemplates['new-module-area'] ).appendTo(row).css({ height : 0 }).animate({
				height : 99
			}, 300, function(){
	...
	function dslc_modules_area_copy(area){
		...
		var dslcModulesAreaCloned = area.clone();
		area.after(dslcModulesAreaCloned);
		...
		dslcModulesAreaCloned.data('size', area.data('size'))
 ...
 ***********************************/
	...
	function dslc_module_width_set(module, newWidth){
		...
		if(DSLC.F.isVer2Edited()){

			DSLC.F.currEditedMod()
			.setOption('dslc_m_size', newWidth)
			.saveEdits();
		...
		// If not slider module
			...
			dslc_module_output_altered(function(){
				...
				if(!jQuery('body').hasClass('rtl')){
					...
					jQuery('.dslca-module-edit-options-inner .jspPane').children().remove();
				}else{
					jQuery('.dslca-module-edit-options-inner').children().remove();
				}

				jQuery('.dslca-module-edit-options-tabs').children().remove();
	...
	function dslc_module_options_cancel_changes(callback)
	{
		...
		jQuery('.dslca-module-options-front', editedModule).children().remove();
		jQuery('.dslca-module-options-front', editedModule).append(originalOptions);
		...
		jQuery(document).on('click', '.dslca-module-edit-field-font-next', function(e){
			...
			if(!jQuery(this).hasClass('dslca-font-loading') && !jQuery(this).siblings('.dslca-font-loading').length){
				...
				if(!DSLC.F.isVer2Edited()){
					...
					var moduleValues = DSLC.F.currEditedMod().values;

					WebFont.load({
						google:{
							families: [dslcAllFontsArray[dslcNewIndex] + ":100,200,300,400,500,600,700,800,900"]
						},
						active: function()
						{
							dslcField.val(dslcAllFontsArray[dslcNewIndex]).trigger('change');
		...
		jQuery(document).on('click', '.dslca-module-edit-field-font-prev', function(e){
			...
			if(!jQuery(this).hasClass('dslca-font-loading') && !jQuery(this).siblings('.dslca-font-loading').length){
				...
				if(!DSLC.F.isVer2Edited()){
					...
					WebFont.load({
						google:{
							families: [dslcAllFontsArray[dslcNewIndex]]
						},
						active: function()
						{
							dslcField.val(dslcAllFontsArray[dslcNewIndex]).trigger('change');
		...
		jQuery(containerClass + ' .dslca-module-edit-field-slider').each(function(){
			...
			dslcSlider.slider({
				...
				slide: function(event, ui){
					...
					DSLC.F.currEditedMod().setOption(dslcOptionID, ui.value);
	...
	function dslc_module_output_altered(callback){
		...
		dslcSettings['action'] = 'dslc-ajax-update-module';
		...
		jQuery.post(
			...
			function(response){
				...
				/* LC 1.5 Ferrum start */
				if(DSLC.commonVars.switchFrom1to2 == true){

					DSLC.commonVars.switchFrom1to2 = false;
					return false;
				}
				/* LC 1.5 Ferrum end */
	...
	jQuery(document).ready(function($){
		 ...
		 */
		 ...
		 $( document ).on( 'click', '.dslca-delete-module-hook', function( e ){
		 	...
		 	var row = jQuery( this ).closest( ".dslc-modules-section-inner" );

			e.preventDefault();
			var ver2module = jQuery( this ).closest( '.dslc-module-front' ).data( 'module-instance' );
	 		...
	 		DSLC.ModalWindow({
				...
				confirm: function()
				{
					DSLC.ModulesManager.removeModule( ver2module );

					setTimeout(function()
					{
						if(row.find(".dslc-module-front").length == 0){

							row.children().remove();
							dslc_add_modules_area( row );
						}

						/// Only on this stage module is removed
						dslc_generate_code();
						dslc_show_publish_button();
					}, 350);
				},
		...
		$(document).on('click', '.dslca-module-edit-hook', function(e){
			...
			if(jQuery('.dslca-module-being-edited, .dslca-modules-section-being-edited').length > 0){
				...
				DSLC.ModalWindow({
					...
					confirm: function()
					{
						if(jQuery('.dslca-modules-section-being-edited').length > 0){

							jQuery('.dslca-row-edit-save').click();
							jQuery('.dslca-modules-section-being-edited').removeClass('dslca-modules-section-being-edited');
						}else{

							jQuery(".dslca-module-edit-save").click();
							jQuery('.dslca-module-being-edited').removeClass('dslca-module-being-edited');
						}
					...
					cancel: function()
					{
						if(jQuery('.dslca-modules-section-being-edited').length > 0){
							...
							jQuery('.dslca-row-edit-cancel').click();
							jQuery('.dslca-modules-section-being-edited').removeClass('dslca-modules-section-being-edited');
						}else{

							jQuery(".dslca-module-edit-cancel").click();
							jQuery('.dslca-module-being-edited').removeClass('dslca-module-being-edited');
						}
			...
			if(dslcDebug) console.time('Cog clicked');
			// Hide (animate) the container
			//jQuery('.dslca-container').css({bottom: -180});
			...
			// Vars
			var dslcDefaultSection = jQuery('.dslca-header').data('default-section');

			var optionsWrapStart = '<div class="dslca-module-edit-options-wrapper dslc-clearfix">';
			var optionsWrapEnd = '</div>';
			...
			var moduleInst = dslcModule.data("module-instance");

			if(dslcDebug) console.time('Settings 2.0 rendered');
			var optionsHtml = moduleInst.renderSettings(); /// OPTIMIZE
			if(dslcDebug) console.timeEnd('Settings 2.0 rendered');

			if(!jQuery('body').hasClass('rtl')){
				var jspane = jQuery('.dslca-module-edit-options-inner .jspPane');

				jspane.children().remove();
				jspane.html(optionsWrapStart + optionsHtml.fields + optionsWrapEnd);

			}else{
				var optInner = jQuery('.dslca-module-edit-options-inner');

				optInner.children().remove();
				optInner.html(optionsWrapStart + optionsHtml.fields + optionsWrapEnd);
			}

			dslc_show_section('.dslca-module-edit');
			var optTabs = jQuery('.dslca-module-edit-options-tabs');

			optTabs.children().remove();
			optTabs.html(optionsHtml.tabs);

			jQuery('.dslca-header .dslca-options-filter-hook').show().eq(0).addClass('dslca-active');
			$(".dslca-module-edit-options-tab-hook").eq(0).addClass('dslca-active').parent().show().siblings().hide();

			setTimeout(function(){

				if(dslcDebug) console.time('Filter options clicked');
				jQuery('.dslca-header .dslca-options-filter-hook').show().eq(0).click(); /// OPTIMIZE
				if(dslcDebug) console.timeEnd('Filter options clicked');
			}, 0);

			DSLC.Mediator.publish('DSLC_tabHooksRendered',
			{
				module: moduleInst
			});
			...
			// Set up backup, if cancel would be clicked
			DSLC.commonVars.currModuleValuesBackup = _.extend({}, moduleInst.values);

			if(dslcDebug) console.timeEnd('Cog clicked');
		});
		...
		$(document).on('click', '.dslca-module-edit-options-tab-hook', function(){
			...
			if(dslcDebug){
				...
				console.time('Tab hook clicked');
			}

			var self = this;
			$(".dslca-module-edit-options-tab-hook").removeClass('dslca-active');
			this.classList.add('dslca-active');
			var uniquePart = this.getAttribute('data-section') + "__" + this.getAttribute('data-id').replace(/[ -]/gi, '_');
			var curOptContClass = ".tab-filter-options-container-" + uniquePart;
			var curOptContainer = $(curOptContClass);
			var module = DSLC.F.currEditedMod();

			curOptContainer.show().siblings().hide();

			if(curOptContainer.html() == ""){

				curOptContainer.html(module.renderSettingsTab(uniquePart));
			}

			var scrollerWidth = 0;

			curOptContainer.prepend(curOptContainer.find(".dslca-module-edit-option-hidden")); /// Make :last-of-type CSS-prop work
			curOptContainer.children().show().each(function(){

				if(!this.classList.contains('dslca-module-edit-option-hidden')){

					scrollerWidth += $(this).outerWidth(true);
				}
			});

			jQuery('.dslca-module-edit-options-wrapper').width(scrollerWidth + 8);
			var jsp = jQuery('.dslca-module-edit-options-inner').data('jsp');
			...
			if(this.getAttribute('data-section') == 'responsive'){

				if(this.getAttribute('data-id') == 'tablet'){

					$('body')
						.removeClass('dslc-res-smaller-monitor')
						.removeClass('dslc-res-phone')
						.removeClass('dslc-res-big')
						.addClass('dslc-res-tablet');
				}else{

					$('body')
						.removeClass('dslc-res-smaller-monitor')
						.removeClass('dslc-res-tablet')
						.removeClass('dslc-res-big')
						.addClass('dslc-res-phone');
				}
			}else{
			...
			setTimeout(function(){
				...
				if(self.getAttribute('data-section') == 'responsive'){
				...
				/**
				 * Set default icon set for icon option field
				 */
				var defIconSet = jQuery(".dslca-module-edit-field-icon-switch-sets .dslca-active").data('set');
				curOptContainer.find(".dslca-module-edit-option-icon").each(function(){
			...
			if(dslcDebug){
				...
				console.timeEnd('Tab hook clicked');
			}
		});
		...
		$(document).on('click', '.dslca-options-filter-hook', function(e){
			...
			if(dslcDebug){
				...
				console.time('Filter hook clicked');
			}

			var dslcPrev = jQuery('.dslca-options-filter-hook.dslca-active').data('section');
			var editingModule = $(".dslca-module-being-edited").data("module-instance");

			$('.dslca-options-filter-hook.dslca-active').removeClass('dslca-active');
			this.classList.add('dslca-active');
			commonVars.filterHookSectionId = this.getAttribute('data-section');

			var sectionId = this.getAttribute('data-section');

			$(".filter-options-container-" + sectionId).show().siblings().hide();
			$(".dslca-module-edit-options-tab-hook").show();

			var curOptTab = $(".tab-filter-container-" + sectionId).show();

			curOptTab.siblings().hide();
			curOptTab.children().eq(0).click();

			// If previous was responsive reload module
			if(dslcPrev == 'responsive' && editingModule && editingModule.settings && !DSLC.F.isVer2Edited()){
				// Show the loader
				jQuery('.dslca-container-loader').show();
			...
			if(dslcPrev == 'responsive'){
				...
				DSLC.F.currEditedMod().reloadModuleBody();
			}

			if(dslcDebug){

				console.timeEnd('Filter hook clicked');
			}
		});
		...
		$(document).on('click', '.dslca-module-edit-cancel', function()
		{
			...
			var curModule = DSLC.F.currEditedMod();
			...
			Object.keys(curModule.values).forEach(function(item){

				curModule.setOption(item, curModule.values[item]);
			});

			curModule.reloadModuleBody();
		});
 ...
 * - dslc_export_template (Export Template)
 *
 ***********************************/
 	...
 	function dslc_template_load(template){
		...
		jQuery.post(
			...
			function(response){
				...
				jQuery('#dslc-main').children().remove();
	...
	jQuery(document).on('click', '.dslca-modal-template-export', function(){
		...
		{
			this.document.body.innerHTML += jQuery('#dslca-code').val();
		};

		/// Chromium
		child.document.body.innerHTML += jQuery('#dslca-code').val();
	});
	...
	function dslc_generate_code(){
		...
		jQuery('#dslc-main .dslc-modules-section').each(function(){
			...
			jQuery('.dslca-modules-section-settings input', modulesSection).each(function(){
				modulesSectionAtts.push(this.getAttribute('data-id') + '="' + jQuery(this).val() + '"');
			});

			// Open the module section (row) shortcode
			composerCode += '[dslc_modules_section ' + modulesSectionAtts.join(' ') + '] ';
			...
			jQuery('.dslc-modules-area', modulesSection).each(function(){
				...
				jQuery('.dslc-module-front', modulesArea).each(function(){
					...
					var attrsStr = '';
					var ver2module = jQuery(this).data('module-instance');
					if(ver2module != null && typeof ver2module.files =='object'){

						Object.keys(ver2module.files).forEach(function(key){

							attrsStr += ' ' + key + '="' + ver2module.files[key] + '" ';
						});
					}

					// Add the module shortcode containing the data
					composerCode = composerCode + '[dslc_module ' + attrsStr + ' last="' + moduleLastState + '" ]' + moduleCode + '[/dslc_module] ';
				});
 ...
 *********************************/
	 ...
	 jQuery(document).ready(function($){
		...
		function ver2saveEditableContent()
		{
			if($(this).closest(".dslc-module-front").data('module-instance') != null && $(this).closest(".dslc-module-front").data('module-instance').settings.version == 2){
				...
				module.setContentEditableValue(this);
			}
		}
		...
		jQuery(document).on('blur', '.dslca-editable-content', function(){
			...
			ver2saveEditableContent.apply(this);
		...
		$(document).on('change', '.dslca-module-edit-field', function(){
			...
			dslcModule.data('module-instance').setOption(dslcOptionID, this.value);
			dslcModule.data('module-instance').reloadModuleBody();
		});
	...
	jQuery(document).ready(function($){
		...
		jQuery(document).on('click', '.dslca-module-edit-field-image-add-hook, .dslca-modules-section-edit-field-image-add-hook', function(){
			...
			file_frame.on('select', function(){
				...
				/// Hook system in work. Fire an event when set image option type value
				DSLC.Mediator.publish('DSLC_setImageOptionValue',
				{
					module: module,
					value: {
						url: attachment.url,
				...
				field.parent().find('.dslca-image-filename').html(filename);

				module.reloadModuleBody();
		...
		jQuery(document).on('click', '.dslca-module-edit-field-image-remove-hook, .dslca-modules-section-edit-field-image-remove-hook', function(){
			...
			field.parent().find('.dslca-image-filename').html('');

			/// Hook system in work. Fire an event when remove image option type value
			DSLC.Mediator.publish('DSLC_removeImageOptionValue',
			{
				module: DSLC.F.currEditedMod(),
				fieldId: field.data('id')
			});
		...
		(function(){
			...
			/// Local variables
			var currentlyEditingField = null;
			...
			$(document).on('click', '.dslca-wysiwyg-actions-edit-hook', function(){
				...
				if(!$(this).closest('.dslc-module-front').hasClass('dslca-module-being-edited')){
					$(this).closest('.dslc-module-front').find('.dslca-module-edit-hook').trigger('click');
				}

				var module = $(this).closest('.dslc-module-front').data('module-instance');
				var content = $(this).parent().siblings(".dslca-editable-content").html();
				var editable = $(this).closest('.lc-editor-element').siblings('.dslca-editable-content');
				currentlyEditingField = $(this).parent().siblings(".dslca-editable-content");
				...
				if(typeof tinymce != "undefined"){
					...
					if($('#wp-dslcawpeditor-wrap').hasClass('tmce-active')){
						...
						editor.setContent(content, {format: 'visual'});
					}else{
			...
			$(document).on('click', '.dslca-wp-editor-save-hook', function(){
				...
				if(typeof tinymce != "undefined"){
					...
					if($('#wp-dslcawpeditor-wrap').hasClass('tmce-active')){
						...
						var content = tinymce.get('dslcawpeditor').getContent();
					}else{
					...
					var optId = $('.dslca-wysiwyg-active').data('id');
					DSLC.F.currEditedMod()
						.setWYSIWIGValue( currentlyEditingField );
 ...
 */
DSLC.F = {
	isVer2Edited: function()
	{
		return jQuery('.dslca-module-being-edited').data('module-instance') != null && jQuery('.dslca-module-being-edited').data('module-instance').settings.version == 2;
	},
	currEditedMod: function()
	{
		return jQuery('.dslca-module-being-edited').data('module-instance');
	}
};